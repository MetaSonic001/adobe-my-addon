...
import { GeminiService, GeneratedContent } from "../services/api";
...
const App = ({ addOnUISdk }: { addOnUISdk: AddOnSDKAPI }) => {
    ...
    const handleGenerateContent = useCallback(async (prompt: string): Promise<GeneratedContent> => {
        ...
        try {
            const content = await geminiService.generateContent(prompt, brandInfo);
            ...
            <div className="container">
                ...
                {!apiKey && (
                    <div className="warning">
                        ⚠️ Set your Gemini API key to get started
                    </div>
                )}
            </div>
        </Theme>
